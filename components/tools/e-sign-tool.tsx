    ...
    <div className="min-h-screen bg-[#fafafa] pb-24">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="inline-flex items-center gap-1 rounded-full border border-slate-200 bg-slate-50 px-2.5 py-0.5 text-xs font-medium text-slate-800">
      ...
      <div className="mx-auto max-w-4xl space-y-6 px-4 py-8 sm:px-6">
        ...
        {!pdfBytes ? (
          <label
            htmlFor={formId}
            className="flex min-h-[200px] cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-200 bg-white px-4 py-12 text-center shadow-sm transition hover:border-slate-300"
          >
            <FileText className="h-10 w-10 text-slate-400" />
            <p className="mt-3 text-sm font-semibold text-black">Upload a PDF (e.g. Aadhar copy)</p>
            <p className="mt-1 text-xs text-slate-500">Max ~18 MB. Everything stays in your browser.</p>
          </label>
        ) : null}
          ...
          <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(280px,340px)]">
            <div className="space-y-4">
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <div className="flex flex-wrap gap-2">
                  <label
                    htmlFor={formId}
                    className="inline-flex cursor-pointer items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-medium text-black hover:bg-slate-50"
                  >
                    Replace PDF
                  </label>
                  <button
                ...
                >
                  {Array.from({ length: numPages }, (_, idx) => (
                    <div key={idx} className="mb-6 w-full">
                      <p className="mb-2 text-center text-xs font-medium text-slate-500">Page {idx + 1}</p>
                      <div
                        data-esign-page-wrap
                        className={`relative mx-auto w-fit max-w-full rounded-lg border border-slate-200 bg-slate-100 shadow-sm ${
                          placementMode !== "idle"
              ...
              {stamps.length > 0 ? (
                <div className="rounded-xl border border-slate-100 bg-white p-4 shadow-sm">
                  <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Placed items</p>
                  <p className="mt-1 text-xs text-slate-500">
                        ...
                        <div className="flex shrink-0 items-center gap-1.5">
                          {s.kind === "image" ? (
                            <>
                              <button
                                type="button"
                                className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-800 hover:bg-slate-100 disabled:opacity-40"
            ...
            <div className="space-y-4 lg:sticky lg:top-6 lg:self-start">
              <div className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
                ...
                <div className="mt-3 flex rounded-lg border border-slate-200 p-0.5 text-xs font-medium">
                  <button
                    type="button"
                    onClick={() => setSigTab("draw")}
                    className={`flex flex-1 items-center justify-center gap-1 rounded-md py-2 ${
                      sigTab === "draw" ? "bg-slate-900 text-white" : "text-slate-600"
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`flex flex-1 items-center justify-center gap-1 rounded-md py-2 ${
                      sigTab === "type" ? "bg-slate-900 text-white" : "text-slate-600"
                    }`}
                  >
                ...
                {sigTab === "draw" ? (
                  ...
                  <div className="mt-3 space-y-2">
                    <label className="block text-xs font-medium text-slate-600" htmlFor={`${formId}-typed`}>
                      Your name
                    </label>
                    <input
                      id={`${formId}-typed`}
                ...
                {activeSigDataUrl ? (
                  <div className="mt-4 space-y-2">
                    <p className="text-xs font-medium text-slate-600">Active signature (drag onto a page)</p>
                    <img
                      ...
                      className="max-h-24 w-auto cursor-grab rounded border border-slate-200 bg-white p-1 active:cursor-grabbing"
                    />
                  </div>
                ) : (
                  <p className="mt-3 text-xs text-slate-500">Save a signature to enable placement on the PDF.</p>
                )}
              </div>

              <div className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
                <p className="text-sm font-semibold text-black">Place on PDF</p>
                <p className="mt-1 text-xs text-slate-500">
                ...
                {placementMode === "attest" ? (
                  <div className="mt-3">
                    <label className="text-xs font-medium text-slate-600" htmlFor={`${formId}-attest`}>
                      Text to stamp
                    </label>
                    <input
                      id={`${formId}-attest`}
                ...
                {placementMode !== "idle" ? (
                  <button
                    type="button"
                    className="mt-3 w-full rounded-lg border border-slate-200 py-2 text-xs font-medium text-slate-700 hover:bg-slate-50"
              ...
              {isPro ? (
                <div className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
                  ...
                  <p className="mt-1 text-xs text-slate-500">Stored only in this browser on this device.</p>
                  ...
                  <ul className="mt-3 max-h-40 space-y-1 overflow-y-auto text-sm">
                    {savedSigs.map((s) => (
                      <li key={s.id} className="flex items-center justify-between gap-2 rounded-lg bg-slate-50 px-2 py-1.5">
                        <button
                          type="button"
                          className="truncate text-left font-medium text-slate-800 hover:underline"
                          onClick={() => {
                ...
                <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50/80 p-4 text-xs text-slate-600">
                  <span className="font-semibold text-slate-800">Daily Pass:</span> save multiple named signatures
      ...
      <DailyPassUpsellModal
        ...
        description={
          <>
            Free signing is limited to <span className="font-medium text-black">page 1</span> and includes a small
            footer credit. A <span className="font-medium text-black">Daily Pass from ₹19</span> removes the
            watermark, lets you stamp <span className="font-medium text-black">every page</span>, and lets you save
